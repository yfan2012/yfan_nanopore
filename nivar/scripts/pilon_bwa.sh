...
do
    ...
    bwa mem -t 36 $1/index/$prefix.fasta $1/*fwd_paired.fq.gz $1/*rev_paired.fq.gz | samtools view -@ 36 -bS - | samtools sort -@ 36 -o $1/bam/$prefix.sorted.bam
    samtools index $1/bam/$prefix.sorted.bam

    ##do the correction
    echo correcting for round $i
    java -Xmx100G -jar ~/software/pilon-1.23.jar --threads 36 --changes --tracks --genome $1/index/$prefix.fasta --frags $1/bam/$prefix.sorted.bam --outdir $1 --output $prefix.pilon_bwa.$i
